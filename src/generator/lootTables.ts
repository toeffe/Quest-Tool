...
import type { Project, ZoneDrop, ZoneDropMode } from '../types/quest';
...
/** Loot context for quest/mob drops: works with /loot spawn, DeathLootTable, and zone drops. */
export const QUEST_LOOT_TABLE_TYPE = 'minecraft:generic';

/** Shared empty loot table (minecraft:empty was removed in 1.21). */
export function buildEmptyEntityLootTable(): LootTableJson {
  return { type: QUEST_LOOT_TABLE_TYPE, pools: [] };
}
...
/** Non-display item components (glint, rarity, enchantments, etc.). */
function customItemStyleComponents(item: CustomItem): Record<string, unknown> {
...
/** JSON components for minecraft:set_components (e.g. non-loot contexts). */
export function customItemComponentsJson(item: CustomItem): Record<string, unknown> {
  const components: Record<string, unknown> = {
    ...
    ...customItemStyleComponents(item),
  };

  if (item.displayName.trim()) {
    components['minecraft:item_name'] = { text: item.displayName.trim(), italic: false };
  }
...
export function customItemLootFunctions(
  ...
  if (item.displayName.trim()) {
    functions.push({
      ...
      name: { text: item.displayName.trim(), italic: false },
      target: 'item_name',
    });
  }
  ...
  if (loreLines.length) {
    functions.push({
      ...
      lore: loreLines.map((line) => ({ text: line.trim(), italic: false })),
      mode: 'replace_all',
    });
  }

  const style = customItemStyleComponents(item);
  if (Object.keys(style).length) {
    functions.push({ function: 'minecraft:set_components', components: style });
  }

  return functions;
}

function dropEntry(project: Project, drop: ZoneDrop): Record<string, unknown> | null {
  ...
  if (drop.customItemId) {
    ...
    functions = customItemLootFunctions(item, amount);
  ...
  if (chance < 100) {
    entry.conditions = [{ condition: 'minecraft:random_chance', chance: chance / 100 }];
  }
...
export function buildZoneDropLootTable(project: Project, drops: ZoneDrop[]): LootTableJson {
