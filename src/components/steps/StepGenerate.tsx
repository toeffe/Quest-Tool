...
export function StepGenerate({ project }: Props) {
  ...
  const [resourcePackDownloaded, setResourcePackDownloaded] = useState(false);
  ...
  async function handleDownloadResourcePack() {
    ...
    try {
      ...
      a.click();
      URL.revokeObjectURL(url);
      setResourcePackDownloaded(true);
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <div>
      ...
      <div className="card">
        ...
        {issues.length === 0 && <div className="success-banner">{t('validation.allGood')}</div>}
      ...
      <div className="card">
        ...
        <div className="toolbar">
          ...
          {hasSkins && (
            <button className="btn" disabled={blocked || busy} onClick={handleDownloadResourcePack}>
